...
for (y in 1974:2025) {
  ...
  temp <- dplyr::select(
    temp, species, tag_code, recovery_id, recovery_date, fishery, gear,
...
recover <- recover |> dplyr::filter(tag_code != "") # %>%
...
release <- readRDS("data/chinook/all_releases_june2026.rds")

release <- dplyr::select(
  release, tag_code_or_release_id, run, brood_year, first_release_date,
...
dat <- rename(dat2, recovery_rmis_region = rmis_region, recovery_rmis_basin = rmis_basin)
